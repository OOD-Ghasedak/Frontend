...
interface OwnedOrManagedChannel {
    id: string;
    name: string;
    description: string;
    is_owner: boolean;
...
export default class OwnedOrManagedChannelRow extends Vue {
  get roleProps () {
    return ChannelRolesProps[this.channelRole]
  }

      @Prop() readonly channel: OwnedOrManagedChannel
      get channelRole () {
        return this.channel.is_owner ? ChannelRole.OWNER : ChannelRole.ADMIN
      }
}
</script>
